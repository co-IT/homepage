...
export const ProductFeatureList = component$((props: ProductFeatureListProps) => {
  return (
    <ul class='grid gap-y-2'>
      {props.features.map((feature, key) => {
        return (
          <li key={key} class='flex gap-2'>
            {feature.detail ? <InfoPopover text={feature.detail} /> : <span class='w-[18px]'></span>}
            {/* margin-top is a hack since the InfoPopover is 24px height
                ...
                */}
            <span class={`mt-[-.25em] text-black ${feature.class}`}>{feature.text}</span>
          </li>
        );
      })}
    </ul>
